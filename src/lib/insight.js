...
Insight.prototype.getTransaction = function(txid, callback) {
  ...
  this.requestGet('/api/tx/' + txid, (err, res, body) => {
    if (err || res.statusCode !== 200) {
      return callback(err || res);
    }
    var tx = JSON.parse(body);

    tx.url = this.url + '/tx/' + txid
 ...
 */
Insight.prototype.getAddress = function(address, callback) {
  $.checkArgument(_.isFunction(callback));
  ...
  this.requestGet('/api/addr/' + address.toString(), function(err, res, body) {
    ...
    var addres = JSON.parse(body);

    return callback(null, addres);
  });
};
